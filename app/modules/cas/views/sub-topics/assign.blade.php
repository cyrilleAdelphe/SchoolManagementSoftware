...
@section('content')
	<a href = "{{URL::route('cas-sub-topics-list')}}" class = "btn btn-info">Go Back To Cas Topics</a>
	
	<div class = "content">
		<form method = "post" action = "{{URL::route('cas-sub-topics-assign-marks-post', $subject_id)}}" id='assignMarksForm'>
			<div class = "row">
				<div class = "col-md-6">
...
<script src="{{ asset('backend-js/submit-enable-disable.js') }}" type="text/javascript"></script>
	<script>
		...
		{	
			...
			if(parseFloat($(this).val()) >parseFloat($('#full_marks').val()) )
			{
				alert('Cas marks are in terms of percentage. The marks exceeds the Full Marks');
				$(this).val($('#full_marks').val() );
			}
