	...
	<form method = "post" action = "{{URL::route($module_name.'-edit-post',$data->id)}}" id = "backendForm">
      <div class="form-group @if($errors->has('title')) {{'has-error'}} @endif">
          ...
          <input id="title" name="title" class="form-control required" type="text" placeholder="Enter event title"
          		value= "{{ (Input::old('title')) ? (Input::old('title')) : $data->title }}">
      ...
      <div class="form-group  @if($errors->has('venue')) {{'has-error'}} @endif">
          ...
          <input name="venue" id="venue" class="form-control required" type="text" placeholder="Enter venue"
          			value= "{{ (Input::old('venue')) ? (Input::old('venue')) : $data->venue }}">
          <span class = 'help-block'>
      ...
      <div class="form-group @if($errors->has('date')) {{'has-error'}} @endif">
        ...
        <div class="input-group">
          ...
          <input name="date" type="text" class="form-control pull-right" id="reservationtime"
                  value= "{{ (Input::old('date')) ? (Input::old('date')) : EventsHelper::convertToRange($data->from_ad,$data->to_ad) }}"/>
        </div><!-- /.input group -->
        <span class = 'help-block'>
      ...
      <div class="form-group @if($errors->has('for')) {{'has-error'}} @endif">
        ...
        <label>
          ...
          <input type="checkbox" name="for_students" value="yes" class="flat-red" {{ (Input::old('for_students'))==='yes'?'checked': ($data->for_students==='yes'?'checked':'') }}/>
          Students
        </label>&nbsp;&nbsp;&nbsp;
        <label>
          <input type="hidden" name="for_teachers" value="no">
          <input type="checkbox" name="for_teachers" value="yes" class="flat-red" {{ Input::old('for_teachers')==='yes'?'checked':($data->for_teachers==='yes'?'checked':'') }}/>
          Teachers
        </label>&nbsp;&nbsp;&nbsp;
        <label>
          <input type="hidden" name="for_management_staff" value="no">
          <input type="checkbox" name="for_management_staff" value="yes" class="flat-red" {{ Input::old('for_management_staff')==='yes'?'checked':($data->for_management_staff==='yes'?'checked':'') }}/>
          Management Staff
        </label>&nbsp;&nbsp;&nbsp;
        <label>
          <input type="hidden" name="for_parents" value="no">
          <input type="checkbox" name="for_parents" value="yes" class="flat-red" {{ Input::old('for_parents')==='yes'?'checked':($data->for_parents==='yes'?'checked':'') }}/>
          Parents
        </label>&nbsp;&nbsp;&nbsp;
        <label>
      ...
      <div class="form-group @if($errors->has('description')) {{'has-error'}} @endif">
          ...
          <textarea class="textarea" name="description" placeholder="Describe your event here" 
          			style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ (Input::old('description')) ? (Input::old('description')) : $data->description }}</textarea>
      	  <span class = 'help-block'>
