@extends('backend.'.$role.'.main')
  ...
  <div class="row">
    <div class="col-sm-6">
      ...
      <div class="row">
        <div class="col-sm-12">
      <h4 class="text-green">Locate other vehicle</h4> 
      <table id="pageList" class="table table-bordered table-striped">
        ...
        <tbody>
          @if(count($transportations))
            @define $i = 1;
            @foreach($transportations as $t)
          <tr>
            <td>{{$i++}}</td>
            <td>{{$t->number_plate}}</td>
            <td>{{$t->route}}</td>
            <td>
              <a data-toggle="tooltip" title="Locate" href="{{URL::route('transportation-view-locations', array($t->unique_transportation_id))}}" class="btn btn-info btn-flat"><i class="fa fa-fw fa-map-marker"></i></a>
            </td>
          </tr>
          @endforeach
          @else
            <tr><td>No Transportations Available</td></tr>
          @endif
