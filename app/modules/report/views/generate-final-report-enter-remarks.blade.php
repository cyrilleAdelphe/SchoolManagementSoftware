...
@section('content')
	...
	@foreach($data as $student_id => $d)
		...
		<div class = "remarks">
			...
			<textarea class = "remarks_value form-control" name = "remarks[{{$student_id}}]">@if(isset($remarks[$student_id]['remarks'])){{$remarks[$student_id]['remarks']}} @else  @endif</textarea>
		</div>

		<div>
			<label>Attendance</label>
			<input type = "text" name = "attendance[{{$student_id}}]" value = "@if(isset($remarks[$student_id]['attendance'])) {{$remarks[$student_id]['attendance']}} @else  @endif">
		</div>
