...
<script>
	// the only difference from the origin implementation in tableSearch.js is that employee_username and _token parameters are added!!!
	...
	$('.search_column').keyup(function(e) {
		...
		if(e.which == 13) {
			...
			temp = url + parameters;
			parameters = addGetParametersToUrl(temp, 'employee_username', "{{ Input::get('employee_username', 0) }}");
			temp = url + parameters;
	...
	function addGetParametersToUrl(address, parameter_name, parameter_value) {
		...
		var employee_username = getUrlParameter(address, 'employee_username');
		...
		// these lines were added for this module
		else if (parameter_name == 'employee_username')
			employee_username = parameter_value
		...
		// these lines were added for this module
		if (typeof employee_username != 'undefined')
			result += 'employee_username=' + employee_username + '&';
