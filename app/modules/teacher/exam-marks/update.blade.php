...
@section('content')
	<form method="post" action="{{URL::route('teacher-exam-update-marks-post')}}">
	<div class="row">
	<div class="col-sm-2">
    ...
    <div class="col-sm-12">
      <h4 class="text-red">Class {{HelperController::pluckFieldFromId('Classes', 'class_name', Input::get('class_id', 0))}} 
	      ...
	      <table id="pageList" class="table table-bordered table-striped">
	        <thead>
	          <tr>
	            ...
	            <th>Marks  FM: {{$full_marks_pass_marks->practical_full_marks}} PM: {{$full_marks_pass_marks->practical_pass_marks}} <input type = "hidden" id = "full_marks" value = "{{$full_marks_pass_marks->practical_full_marks}}">
	            <input type = "hidden" id = "pass_marks" value = "{{$full_marks_pass_marks->practical_pass_marks}}">
	            </th>
	            @endif
	            <th>Comments</th>
	          </tr>
	        ...
	        <tbody>
	        	...
	        	@else
	        		...
	        		@foreach($student_marks as $student)
	        			...
	        			<td>
	        				<input name="marks[]" class="form-control obtained_marks" type="text" value="{{$student->marks}}">
	        			</td>
	        			@if($full_marks_pass_marks->practical_full_marks)
	        			<td>
			  ...
			  <input type="hidden" id="subject_ajax" value="{{URL::route('teacher-subjects-from-exam-class')}}" />
	      ...
	      <div class="form-group">
	        <button type="submit" id="sub-btn" class="btn btn-success" @if(AccessController::checkPermission('exam-marks', 'can_edit') == false) disabled @endif >Save</button>
	      </div>
...
<script>
	...
	function updateClassList(exam_id, default_class)
  {
    ...
    $.ajax( {
                      "url": "{{URL::route('teacher-classes-from-exam')}}",
  ...
  $(function() {
  	...
  	updateClassList("{{ Input::get('exam_id', 0) }}", "{{ Input::get('class_id', 0) }}");
	...
	$('#class_id').change(function()
	    {
	      var class_id = $(this).val();
	        ...
	        $.ajax( {
	                      "url": "{{URL::route('teacher-exam-show-class-section-list')}}",
	    ...
	    $('.obtained_marks').keyup(function(e)
	    {
