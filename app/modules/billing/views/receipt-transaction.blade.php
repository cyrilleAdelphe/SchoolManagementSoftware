...
@section('content')
              ...
              <div class="row">
                <div class="col-sm-3">
                  <div class="form-group">
                    <label>Choose Receipt No</label>
                    <input type = 'number' name = 'start' value="Show current date" id = "myDate" class = 'form-control myDate required'>
                  </div>
                </div>
                <div class="col-sm-3">
                  <label style="color: #fff; display: block;">Generate</label>
...
<script type="text/javascript">
        ...
        $('#ajax-content').html('<div class="dloading"><img src="{{ asset('sms/assets/img/loading.gif') }}"><br/>loading...</div>');
          $.ajax({
            'url' : '{{URL::route('api-billing-ajax-show-receipts-get')}}',
            'method' : 'GET',
            'data' : {'date_range' : date_range, 'type' : 'date'}
          }).done(function(data)
          {
