			...
			@foreach($data['data'] as $d)
				...
				<div class="happy-info" style="margin-bottom: 10px">
			        Details of <strong>{{$invoice_details['personal_details']['name']}} </strong>| @if($invoice_details['personal_details']['group'] == 'student')
						...
						Class - Section : {{$invoice_details['personal_details']['class']}} - {{$invoice_details['personal_details']['section']}}
					@endif
					Invoice Number : <strong>{{$d->financial_year}}-{{$d->invoice_number}}</strong>
			    </div>
					...
					@if($invoice_details['personal_details']['group'] == 'student')
						@if(isset($invoice_details['discount']))
							...
							@foreach($invoice_details['discount'] as $dis)
								<div class="info-bar" >
									Discount Details
								</div>
								<table class = "table table-striped table-hover table-bordered">
									<tr>
										...
										<td>{{$dis['organization_name']}} - {{$d['discount_title']}}</td>
									</tr>
									<tr>
					...
					<a class="btn btn-lg btn-flat btn-danger pull-right">Paid Amount: {{$d->received_amount}}</a>
			</div>
			<br>
