{{ Form::open(array('url' => URL::route($module_name . '-file-post'),'method'=>'POST', 'files'=>true,  'id' => 'backendUploadForm')) }}
  ...
  <div class="form-group {{ ($errors->has('filename')) ? 'has-error' : ''}}">
	...
	<div class = "form-group">
		<button class="btn btn-flat btn-success submit-enable-disable" type = "submit" related-form = "backendUploadForm"> <i class="fa fa-fw fa-upload"></i> Upload </button>
	</div>

	<input type = "hidden" name = "student_id" value = "{{ $data['student']->id }}" />
	<input type = "hidden" name = "student_username" value = "{{ $data['student']->username }}" />
